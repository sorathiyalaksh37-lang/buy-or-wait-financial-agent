import sys
from datetime import date
from pathlib import Path
sys.path.append(str(Path(__file__).parent.parent))
from ingestion import load_dataset
from forecaster import Forecaster
from extraction import Extractor

extractor = Extractor()
ds = load_dataset(Path("dataset"), extractor)

# request_16: user_16, INR, balance=362370, min=122400, safe=0?
state = ds.get_user_state("user_16")
fc = Forecaster(state, date(2023, 8, 12))

print("Balance:", state.balance)
print("Min balance:", state.min_balance)
print()
print("Pending/scheduled events (will affect balance):")
for e in state.clean_events:
    if e.status != "settled":
        print(f"  {e.event_id} {e.event_date} {e.status} {e.direction} {e.amount} {e.currency} {e.category}")

print()
print("Recurring:")
for r in state.recurring_expenses:
    print(f"  {r.category} ~{r.avg_amount:.0f} every {r.cadence_days}d")

print()
print("Daily flows forward:")
running = state.balance
for d, amounts in sorted(fc.daily_flows.items()):
    total = sum(amounts)
    running += total
    print(f"  {d}: {total:+.2f} => {running:.2f}")
